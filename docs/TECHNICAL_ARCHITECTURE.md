...
Conditions can now return a contextual result in addition to their raw boolean. The result identifies the condition node and block, the checked subject and subject kind, the raw true/false result, and a readable fact summary. `condition.player.has_tag`, `condition.player.is_admin`, and `condition.context_entity.has_tag` provide this result on both true and false evaluation. `PASS_ONLY`, `FAIL_ONLY`, and `BRANCH` keep their existing routing semantics; whichever path actually continues receives the complete result, including a false-path subject.

The current condition result belongs to one run and its current control path. A later ordinary condition replaces it; a condition that cannot provide a contextual result clears the previous value rather than leaking stale context. Loop-until rack predicates read the current runtime entity context but do not replace the ordinary path's current condition result. No condition result, current entity, subject identity, or entity frame is written to graph JSON, drafts, storage, autosave, or undo/redo, and there is no process-global `lastConditionResult` or `currentEntity`.

The catalog adds the `执行上下文` category and a single-body C-shaped `context.entity.execute_as` block. Its `entitySource` is one of `CONDITION_SUBJECT` (`当前条件对象`), `RUN_ENTITY` (`运行实体`), or `TARGET_ENTITY` (`目标实体`). Entry resolves exactly the selected source, saves the previous current entity, runs `body`, restores the previous entity on natural completion, and follows `done`; an empty body performs the same enter/restore as a no-op. This is execute-as semantics only. It does not change position, dimension, rotation, or any other execute-at state.

The existing execution cursor and continuation path carry the run entity, optional target entity, current entity, current condition result, and entity-context frames alongside loop frames. This preserves the selected entity through nested context/loop combinations and `timer.wait`; completion unwinds the actual mixed scope order. Resume validates graph identity, subject/entity resolvability, frame membership, nesting, generation, and single-consumption continuation identity before restoring. Cancelled or stale continuations cannot mutate an entity.

Simulation Test Context may provide one optional target entity with an entity type id, display name, and tags. The run actor remains the run entity and initial current entity; actor and target mutable tag sets are copied for each run. Results report initial/final actor tags separately from initial/final target-entity tags. `condition.context_entity.has_tag`, `action.context_entity.add_tag`, and `action.context_entity.remove_tag` always use the current entity context, while existing `player.*` blocks continue to use the run actor.

Graph persistence stores only the new block ids, `entitySource`, ordinary `body` membership, and typed edges. Save-time validation rejects unknown sources, invalid membership, ancestry cycles, excessive nesting, and invalid tag/output configuration; an empty body remains a warning. Runtime fails closed for missing condition/run/target entities, non-entity condition subjects, unresolvable entities, invalid frames, and stale continuations without falling back to another source.

This checkpoint does not implement execute-at, a real Minecraft entity adapter, player/world scanning, detectors, event listeners, entity fan-out, or cross-restart continuation. A future detector must create one isolated condition result and control path/run per matched entity instead of placing many entities in a shared current-entity array.
