...
- The 25 current concrete block ids are: `trigger.manual_test`, `condition.state.equals`, `condition.entity.has_tag`, `condition.player.is_admin`, `condition.player.dimension_is`, `condition.player.in_region`, `condition.player.y_compare`, `condition.target_block.is_type`, `condition.target_block.in_region`, `condition.target_block.y_compare`, `condition.player.near_target_block`, `control.loop.count`, `control.loop.forever`, `control.loop.until`, `context.entity.execute_as`, `action.entity.add_tag`, `action.entity.remove_tag`, `action.message.chat`, `action.message.title`, `action.message.subtitle`, `action.message.actionbar`, `state.set`, `state.add`, `timer.wait`, and `debug.log`.
...
- Entity tag condition/actions share one simulation/runtime execution path selected through EntityTargetRef.
- `SimulationExecutionRegistry` delegates generic entity-tag behavior so `SimulationRunner` does not become a giant service.
...
- v1 supports only AND across configured slots plus independent per-slot NOT. The current predicate set is `condition.entity.has_tag`, `condition.player.is_admin`, `condition.player.dimension_is`, `condition.player.in_region`, and `condition.target_block.is_type`.
...
## Entity Target Reference + Entity Execution Context v1

Actions and conditions select one entity through a shared `EntityTargetRef`. Its four sources are `CURRENT_ENTITY`, `CONDITION_SUBJECT`, `TARGET_ENTITY`, and `ONLINE_PLAYER`; each block declares `ANY_ENTITY`, `LIVING_ENTITY`, or `PLAYER_ONLY`. There is no permanent start-of-run entity source. A trigger may initialize the optional current entity, and a run may also begin without any entity context.

Persisted Graph config stores one nested `target` object. The in-memory string-backed config map uses a focused serializer at the Graph JSON boundary; loose target source/UUID/name sibling fields and implicit missing-target defaults are not accepted. Online-player identity is the canonical UUID. The saved name is display-only: execution never falls back by name, never rebinds a changed identity, and never operates on an offline player.

The shared resolver reads exactly the configured source and reports stable structured errors for invalid config, missing/unresolvable/offline targets, unavailable providers, type mismatch and not-alive requirements. A target resolution error terminates the action, condition, Predicate Rack evaluation or execute-as entry; it is not converted into a normal false result. Normal generic tag-condition true and false results both record the resolved entity as `RuntimeConditionResult.subject`; condition object remains absent and `targetEntity` stays independent.

The three generic tag blocks are `condition.entity.has_tag`, `action.entity.add_tag`, and `action.entity.remove_tag`. They share the same target resolver and tag execution path. Tag actions return a typed outcome: actual change and idempotent no-change are both `SUCCESS` with `affectedCount=1`; target resolution failure is `FAILURE` with `affectedCount=0`. The six retired player/context variants have no alias, migration or Catalog fallback.

The single-body C-shaped `context.entity.execute_as` block uses the same composite `target` field and defaults to `CONDITION_SUBJECT`. Entry resolves exactly the selected source, saves the outer current entity, runs `body`, restores the previous value on natural completion, and follows `done`; an empty body performs the same enter/restore as a no-op. This is execute-as semantics only. It does not change position, dimension, rotation, or any other execute-at state.
...
Simulation Test Context may initialize the current entity, provide one optional target entity and expose one bounded online-player fixture. Mutable tag sets are copied for each run. The Fabric adapter resolves current online players by UUID and exposes only UUID/name listing through `GET /api/pixellogic/runtime/online-players`; it does not enumerate worlds or load chunks.

This checkpoint does not implement execute-at, general non-player world lookup, selectors, player/world scans, detectors, event listeners, entity fan-out, offline-player actions or cross-restart continuation. The next slice is Health and Termination after automatic validation and the explicit user hand-test gate.
