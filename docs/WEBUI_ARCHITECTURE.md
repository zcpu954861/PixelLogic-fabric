...
- `action.message.chat` uses a shared `rich_text_component` editor: multiline continuous text editing, selected-text color/style controls, structured storage, and an approximate preview. The normal UI does not expose raw JSON or component/segment management.
...
- `ui/editor/richText/`: shared rich text editor toolbar and preview rendering.
