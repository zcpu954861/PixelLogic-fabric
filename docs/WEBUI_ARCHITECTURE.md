...
The Vite dev server proxies `/api` to `http://127.0.0.1:18111`. The UI defaults to `WebUI 模拟玩家`, and the right panel can edit a per-run `测试玩家` context before starting a test.
...
- The `测试玩家` card sends display name, tags, and administrator status with the test run; final tags from `action.player.add_tag` stay in the result summary and do not rewrite the input tags.
...
- `ui/simulation/`: compact test-player panel and simulation result summary.
