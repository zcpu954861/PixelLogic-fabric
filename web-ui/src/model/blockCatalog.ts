...
export const fallbackCatalog: BlockCatalog = {
  ...
  blocks: [
    ...
    block('condition.player.has_tag', '玩家拥有标签', '当当前模拟玩家拥有指定标签时走通过分支。', 'player', 'player.tag', 'condition', 'PLAYER_HAS_TAG_CONDITION', { tag: 'runner' }, [input('input')], [out('pass'), out('fail')], [
      field('tag', '标签', 'string'),
